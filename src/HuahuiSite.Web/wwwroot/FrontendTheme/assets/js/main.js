(function ($) {
    ...
    $('.product_column1').owlCarousel({
     ...
     /*---carousel column3 activation---*/
     $('.carousel_column3').on('changed.owl.carousel initialized.owl.carousel', function (event) {
        $(event.target).find('.owl-item').removeClass('last').eq(event.item.index + event.page.size - 1).addClass('last')}).owlCarousel({
        autoplay: true,
		loop: true,
        nav: true,
    ...
    /*---testimonial container activation---*/
   ...
   $('.mini_cart_wrapper > a').on('click', function(event){
	...
	var categoryToggle = $('.category_toggle > a');
	var categoryMenu = $('.categories_menu_toggle');
